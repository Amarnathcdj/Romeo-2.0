...
# from Romeo.modules.sql import warns_sql as warnssql
import Romeo.modules.sql.blacklist_sql as blacklistsql
from Romeo.modules.sql import disable_sql as disabledsql

# from Romeo.modules.sql import cust_filters_sql as filtersql
# import Romeo.modules.sql.welcome_sql as welcsql
import Romeo.modules.sql.locks_sql as locksql
from Romeo.modules.connection import connected
...
def import_data(update, context):
    ...
    if msg.reply_to_message and msg.reply_to_message.document:
        ...
        try:
            ...
            msg.reply_text(
                "An error occurred while recovering your data. The process failed. If you experience a problem with this, please take it to @Romeo_JulietBotSupport."
            )
...
def export_data(update, context):
    ...
    f = open("Romeo{}.backup".format(chat_id), "w")
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("Romeo{}.backup".format(chat_id), "rb"),
        caption="*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `Romeo-Backup` was specially made for notes."
        .format(chat.title, chat_id, tgl),
        timeout=360,
        reply_to_message_id=msg.message_id,
        parse_mode=ParseMode.MARKDOWN,
    )
    os.remove("Romeo{}.backup".format(chat_id))  # Cleaning file
